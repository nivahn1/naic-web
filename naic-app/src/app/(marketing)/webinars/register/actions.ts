...
const WEBINAR_SLUGS = WEBINARS.map((w) => w.slug) as [string, ...string[]];
const COUNTRY_CODES = COUNTRIES.map((c) => c.code) as [string, ...string[]];
const REGISTRATION_PRICE_CENTS = 9900;
...
const RegistrationSchema = z.object({
  webinar_slugs: z
    .array(z.enum(WEBINAR_SLUGS))
    .min(1, { error: "Choose at least one webinar." }),
  full_name: z
    .string()
    .trim()
...
export async function submitRegistration(
  ...
  const parsed = RegistrationSchema.safeParse({
    webinar_slugs: formData.getAll("webinar_slugs"),
  ...
  const webinars = parsed.data.webinar_slugs.map(
    (slug) => WEBINARS.find((w) => w.slug === slug)!,
  );
  const totalCents = webinars.length * REGISTRATION_PRICE_CENTS;
  ...
  const { error } = await supabase.from("webinar_registrations").insert({
    id: registrationId,
    webinar_slugs: webinars.map((w) => w.slug),
    webinar_names: webinars.map((w) => w.title),
  ...
  if (!isStripeConfigured || !stripe) {
    ...
    redirect(`/webinars/register/success?registration_id=${registrationId}`);
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: webinars.map((w) => ({
      price_data: {
        currency: "usd",
        unit_amount: REGISTRATION_PRICE_CENTS,
        product_data: {
          name: `${w.title} — Webinar Registration`,
          description: "National AI Consortium webinar registration",
        },
      },
      quantity: 1,
    })),
    billing_address_collection: "auto",
    success_url: `${origin}/webinars/register/success?registration_id=${registrationId}&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${origin}/webinars/register?webinar=${webinars[0].slug}&cancelled=1`,
    metadata: {
      registration_id: registrationId,
      webinar_slugs: webinars.map((w) => w.slug).join(","),
    },
  });

...
async function confirmPayment(registrationId: string, sessionId: string) {
  ...
  await admin
    .from("celebration_registrations")
    .update({
      status: "paid",
...
export default async function RegistrationSuccessPage({
  ...
  if (registration_id && isSupabaseAdminConfigured) {
    ...
    const { data } = await admin
      .from("celebration_registrations")
      .select("id, celebration_names, full_name, email, status, amount_cents")
          ...
          {registration ? (
            <>
              <p className="font-display text-lg font-semibold text-white">
                {registration.celebration_names.join(", ")}
              </p>
              <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
                {paid ? (
                  <>
                    ...
                    {registration.celebration_names.length === 1 ? "" : "s"}{" "}

...
export function RegistrationForm({
  ...
  defaultWeekSlug: string;
}) {
  const [state, formAction] = useActionState<RegistrationResult, FormData>(
    submitRegistration,
    {},
  );
  const [selected, setSelected] = useState<string[]>([defaultWeekSlug]);
    ...
    >
      ...
      <fieldset>
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          Select one or more — $599 each. You&rsquo;ll pay for all of them
          ...
          {WEEKS.map((w) => (
            ...
            >
              <input
                ...
                name="week_slugs"
                value={w.slug}
                checked={selected.includes(w.slug)}
        ...
        <p className="mt-4 text-sm text-[var(--muted)]">
          ...
          for {selected.length} week{selected.length === 1 ? "" : "s"}
        </p>
      </fieldset>

...
export default async function RegistrationSuccessPage({
  ...
  if (registration_id && isSupabaseAdminConfigured) {
    ...
    const { data } = await admin
      .from("week_registrations")
      .select("id, week_names, full_name, email, status, amount_cents")
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
                {paid ? (
                  <>
                    ...
                    payment went through and your registration
                    {registration.week_names.length === 1 ? "" : "s"} are
                    confirmed. A receipt is on its way to {registration.email}.
                  </>
                ) : confirmError ? (
                  <>

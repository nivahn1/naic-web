...
const WEEK_SLUGS = WEEKS.map((w) => w.slug) as [string, ...string[]];
const COUNTRY_CODES = COUNTRIES.map((c) => c.code) as [string, ...string[]];
const REGISTRATION_PRICE_CENTS = 59900;
...
const RegistrationSchema = z.object({
  week_slugs: z
    .array(z.enum(WEEK_SLUGS))
    .min(1, { error: "Choose at least one AI Week." }),
  full_name: z
    .string()
    .trim()
...
export async function submitRegistration(
  ...
  const parsed = RegistrationSchema.safeParse({
    week_slugs: formData.getAll("week_slugs"),
  ...
  const weeks = parsed.data.week_slugs.map(
    (slug) => WEEKS.find((w) => w.slug === slug)!,
  );
  const totalCents = weeks.length * REGISTRATION_PRICE_CENTS;
  ...
  const { error } = await supabase.from("week_registrations").insert({
    id: registrationId,
    week_slugs: weeks.map((w) => w.slug),
    week_names: weeks.map((w) => w.name),
  ...
  if (!isStripeConfigured || !stripe) {
    ...
    redirect(`/weeks/register/success?registration_id=${registrationId}`);
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: weeks.map((w) => ({
      price_data: {
        ...
        product_data: {
          name: `${w.name} — Registration`,
          description: "National AI Consortium AI Week registration",
        },
      },
      quantity: 1,
    })),
    billing_address_collection: "auto",
    success_url: `${origin}/weeks/register/success?registration_id=${registrationId}&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${origin}/weeks/register?week=${weeks[0].slug}&cancelled=1`,
    metadata: {
      registration_id: registrationId,
      week_slugs: weeks.map((w) => w.slug).join(","),
    },
  });

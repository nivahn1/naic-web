...
export function RegistrationForm({
  ...
  defaultTrainingSlug: string;
}) {
  const [state, formAction] = useActionState<RegistrationResult, FormData>(
    submitRegistration,
    {},
  );
  const [selected, setSelected] = useState<string[]>([defaultTrainingSlug]);
    ...
    >
      ...
      <fieldset>
        ...
        <p className="mt-4 text-sm text-[var(--muted)]">
          ...
          for {selected.length} training{selected.length === 1 ? "" : "s"}
        </p>
        <p className="mt-2 text-sm text-[var(--muted)]">
